...
class RoomSerializer(serializers.ModelSerializer):
    ...
    homeowner = serializers.PrimaryKeyRelatedField(
        label='房主', queryset=Player.objects.all())
    password = serializers.CharField(
        label='房间密码', required=False, max_length=8, write_only=True)
    max_member_count = serializers.IntegerField(
        label='最大人数', min_value=2, max_value=10)
    ...
    def add_member(self, room_json):
        ...
        # TODO: 这里直接返回Player
        room_json['member_list'] = RoomMemberSerializer(
